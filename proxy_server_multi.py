...
def dns_query_over_tun(host: str, qtype: int, dns_server: str, timeout: float, tun_dev: str = "tun0") -> str | None:
    import random
    sock = None
    try:
        ...
        tun_bytes = tun_dev.encode()
        try:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_BINDTODEVICE, tun_bytes)
        except OSError as e:
...
def resolve_dns_over_tun(host: str, dns_server: str = "8.8.8.8", timeout: float = 3.0, tun_dev: str = "tun0") -> str | None:
    try:
    ...
    try:
        ...
        pass
    return dns_query_over_tun(host, 1, dns_server, timeout, tun_dev) or dns_query_over_tun(host, 28, dns_server, timeout, tun_dev)

def create_connection(address: tuple[str, int], timeout: float = 20, tun_dev: str = "tun0") -> socket.socket:
    host, port = address
    resolved_ip = resolve_dns_over_tun(host, tun_dev=tun_dev)
    if resolved_ip:
        host = resolved_ip
...
def proxy_client(client: socket.socket, address: tuple[str, int], tun_dev: str = "tun0") -> None:
    try:
        ...
        if first == b"\x05":
            socks5_client(client, first, tun_dev)
        else:
            http_client(client, first, tun_dev)
    except Exception as e:
        err_msg = str(e)
        if "[错误代码" in err_msg:
...
def start_proxy_server(host: str, port: int, tun_dev: str = "tun0") -> None:
    ...
    while True:
        try:
            ...
            def run_client(tun=tun_dev):
                try:
                    proxy_client(client, address, tun)
                finally:
